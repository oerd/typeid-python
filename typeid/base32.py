...
# Byte-to-index lookup table for the base32 alphabet (0xFF marks invalid bytes).
# fmt: off
TABLE = [
...
def encode(src: bytes) -> str:
    ...
    if len(src) != 16:
        raise ValueError(f"Expected 16 bytes, got {len(src)}.")
...
def decode(s: str) -> bytes:
    if len(s) != SUFFIX_LEN:
        raise SuffixValidationException(f"Suffix must be {SUFFIX_LEN} characters: {s!r}.")

    v = bytes(s, encoding="utf-8")

    if any(TABLE[byte] == 0xFF for byte in v):
        raise SuffixValidationException(f"Suffix contains characters outside the base32 alphabet: {s!r}.")

    # The overflow rule: 26 base32 characters can express more than 128 bits,
    # so the first character must not be greater than "7".
    if TABLE[v[0]] > 0x07:
        raise SuffixValidationException(f"Suffix overflows 128 bits, the first character must be 0-7: {s!r}.")
